<?php
    ...
    if(isset($_POST['cancel']))
    {
        ...
        echo "start db task";
        $date = date("Y-m-d");
        $sql = "insert into book_ad (owner, bookname, writer, edition, adtype, unitprice, copy, totalprice, details, house, road, area, subdistrict, district, postcode, phoneno, email, date, timefrom, timeto) values ('$user', '$bookname', '$writername', $edition, '$adType', $price, $copy, $total_price, '$details', '$house', '$road', '$area', '$subdistrict', '$district', '$postcode', '$phnNo', '$email', '$date', '$timeFrom', '$timeTo')";
        $stat = exe_query_id($sql);
        //$con = get_connection();
        $id = $stat;
        //echo $id;
        ...
        echo $filepath;
        if(move_uploaded_file($_FILES['image1']['name'], $filepath)){
            $sql = "insert into ad_image (ad_id, image_path_1) values ('$id','$filepath')";
            exe_query($sql);
